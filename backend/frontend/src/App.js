...
function App() {
  ...
  const handleEditPin = async (id) => {
    try {
      const pinToEdit = pins.find((pin) => pin._id === id);
      if (pinToEdit.username === myStorage.getItem("user")) {
        setEditPin(true);
        setTitle(pinToEdit.title);
        setDesc(pinToEdit.desc);
        setRating(pinToEdit.rating);
        setCurrentPlaceId(id);
      }
    } catch (err) {
      console.log(err);
  ...
  const handleEditSubmit = async (id) => {
    ...
    try {
      const res = await axios.put(`/pins/${id}`, editedPin);
      setPins((prevPins) =>
        prevPins.map((pin) => (pin._id === id ? res.data : pin))
      );
      setCurrentPlaceId(null);
      setEditPin(false); 
    } catch (err) {
...
<>          
          ...
          { editPin ? (
            <div className="card">
            <form onSubmit={() => handleEditSubmit(p._id)}>
              <label>地點</label>
              <input
                value={title}
            ...
            <label>資訊</label>
              <span className='username'>由{ p.username ? (<b>{p.username}</b>) : (<b>匿名</b>)}創建</span>
